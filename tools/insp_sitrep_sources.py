...
from tools.lib.schema import ALIASES_CSV, REPO_ROOT, to_canonical
...
REPORT_TITLE_RE = re.compile(r"SitRep\s+MVE\s+N[°º]\s*0*(?P<number>\d+)/2026", re.I)
PDF_HREF_RE = re.compile(r"""href=["'](?P<href>[^"']+\.pdf)["']""", re.I)
PDF_URL_TEXT_RE = re.compile(r"https://insp\.cd/wp-content/uploads/[^\s\"'<>]+\.pdf", re.I)
SITREP_LABEL_RE = re.compile(r"SitRep\s+N[°º]\s*0*(?P<number>\d+)/(?:MVB_)?(?P<day>\d{1,2})/2026", re.I)
...
def _extract_pdf_url(post: dict) -> str:
    ...
    if match:
        return urllib.parse.urljoin(post.get("link", "https://insp.cd/"), match.group("href"))
...
def discover_sources(limit: int = 100, *, allow_insecure_tls: bool = False) -> list[ReportSource]:
    query = urllib.parse.urlencode({"search": "SitRep MVE", "per_page": str(limit)})
    ...
    for item in search_results:
        ...
        if not match:
            continue
        report_number = int(match.group("number"))
        post_id = int(item["id"])
        post = _json_from_url(
...
def enrich_pdf_metadata(
    ...
    *,
    ...
    for source in sources:
        ...
        elif download_missing and source.pdf_url and not raw_path.exists():
...
def _national_values_status(parsed: dict[str, str]) -> str:
    values = [parsed.get(field, "") for field in NATIONAL_BY_PARSED_FIELD]
...
def _extract_aligned_cumulative_banner(text: str) -> dict[str, str]:
    ...
    for metric, token in _ordered_headline_values(anchors, value_tokens).items():
    ...
    return extracted if {"confirmed_cases", "confirmed_deaths", "suspected_cases"} <= extracted.keys() else {}
        ...
        if path.exists():
            with path.open(newline="", encoding="utf-8-sig") as f:
                for row in csv.DictReader(f):
                    ...
                    row_index.setdefault((to_canonical(nom) or nom, date_key), len(rows))
...
def write_processed_review(
    ...
    *,
    ...
    for value in draft_values:
        by_key[(value.metric, value.nom, _date_key(value.date))] = value
...
def _processed_index(path: Path, metric: str) -> dict[tuple[str, str], str]:
    with path.open(newline="", encoding="utf-8-sig") as f:
        ...
        return {
            (to_canonical(row["nom"]) or row["nom"], _date_key(row["date"])): row[metric]
            for row in reader
